module chat_app::chat_contract {
    ...
    // 管理者代替用戶標記訊息為已讀（由管理者支付 gas）
    public fun mark_message_as_read_sponsored(
        room: &mut ChatRoom,
        message_index: u64,
        ...
        // 檢查使用者是否已經標記過
        let already_read = vector::contains(&msg.read_by, &reader_address);
        
        if (!already_read) {
            // 將讀者地址加入到已讀列表
            vector::push_back(&mut msg.read_by, reader_address);
    ...
    // 管理者代替用戶批量標記所有訊息為已讀（由管理者支付 gas）
    public fun mark_all_messages_as_read_sponsored(
        room: &mut ChatRoom,
        reader_address: address,
        clock: &Clock,
        _ctx: &mut TxContext
        ...
        while (i < messages_len) {
            ...
            // 檢查使用者是否已經標記過，且不是自己發送的訊息
            let already_read = vector::contains(&msg.read_by, &reader_address);
            if (!already_read && msg.sender != reader_address) {
                vector::push_back(&mut msg.read_by, reader_address);
                // 發送已讀事件
                event::emit(MessageRead {
                    message_index: i,
